...
* [How to Set Up a SharePoint News Feed](#Setup)
  + [Step 1: Setting Up a SharePoint API Gateway](#Gateway)
    - [Choosing the Correct URL Endpoint for SharePoint Connection](#Endpoint)
...
Creating news feeds with OptiSync has a key advantage: you have full control of the look and feel of your feed. You can create feeds that match your company branding and design guidelines no matter which news source you use
...
How to Set Up a SharePoint News Feed
------------------------------------

In order to set up a SharePoint news feed, you'll need to follow these steps:

1. Set Up the SharePoint API Gateway
2. Map the API to a DataSource within OptiSigns
3. Create a News Feed Using OptiSigns Designer

### Step 1: Setting Up a SharePoint API Gateway
...
These are required for authentication. Any further authentication can be done in the Pre-Request stage to obtain access tokens.
...
The easiest way to set up a SharePoint News feed is with a **Data Repeater**. For a full breakdown of a Repeater's capabilities, [see this article](https://support.optisigns.com/hc/en-us/articles/29217646663187). Here, we'll stick to teaching how to add information from your SharePoint API.
...
| --- |
| **TIP** |
| Remember the “PromotedState” data value from before? If you want to make sure only SharePoint news articles appear on your screen, try setting the Filter to state:  **PromotedState - Equals - 2**  This should filter out any other pieces of data received from the API. |
